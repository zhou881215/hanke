/*
 * @Author: Cram
 * @Date: 2022-06-27 17:39:56
 */
import { reactive } from "vue";
import type { FormRules } from "element-plus";

export const ProColumn = [
  { prop: "xh", label: "序号", width: 100 },
  { prop: "lb", label: "类别", width: 150, select: true },
  { prop: "cpmc", label: "产品名称", width: 200 },
  { prop: "jcxm", label: "检测项目", width: 200 },
  { prop: "xmbm", label: "项目别名", width: 200 },
  { prop: "jcbz", label: "检测标准", width: 200 },
  { prop: "bzmc", label: "标准名称", width: 200 },
  { prop: "jcx", label: "检出限", width: 200 },
  { prop: "pdbz", label: "判定标准", width: 200 },
  { prop: "ypxq", label: "样品需求", width: 200 },
  { prop: "zxypl", label: "最小样品量", width: 200 },
  { prop: "csyq", label: "测试仪器实验方法", width: 200 },
  { prop: "zqlx", label: "周期类型", width: 200 },
  { prop: "jczq", label: "检测周期", width: 200 },
  { prop: "bz", label: "报价", width: 200 },
  { prop: "cb", label: "成本", width: 200 },
  { prop: "cma", label: "CMA资质", width: 200 },
  { prop: "cnas", label: "CNAS资质", width: 200 },
  { prop: "jdxz", label: "接单须知", width: 200 },
  { prop: "gys", label: "供应商名称", width: 200 },
  { prop: "sfzy", label: "是否制样", width: 200 },
  { prop: "zyzq", label: "制样周期", width: 200 },
  { prop: "ztfy", label: "制样费用", width: 200 },
];

export const addProductRules = reactive<FormRules>({
  jcxm: [{ required: true, message: "请输入检测项目", trigger: "blur" }],
  jcbz: [{ required: true, message: "请输入检测标准", trigger: "blur" }],
  cma: [{ required: true, message: "请输入CMA资质", trigger: "blur" }],
  cnas: [{ required: true, message: "请输入CNAS资质", trigger: "blur" }],
  gys: [{ required: true, message: "请输入供应商名称", trigger: "blur" }],
});
